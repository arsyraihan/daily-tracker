...
const handleExport = (form) => {
    const url = route('manager.reports.export') + '?' + new URLSearchParams(form.data()).toString();
...
const handlePreview = (form) => {
    ...
    previewUrl.value = route('manager.reports.export') + '?' + params.toString();
...
const tabInfo = computed(() => {
    switch (activeTab.value) {
        case 'attendance': return { title: 'Rekap Absensi', icon: ClipboardCheck, color: 'text-indigo-600', bg: 'bg-indigo-100 dark:bg-indigo-500/20' };
        case 'missions': return { title: 'Laporan Tugas', icon: ListTodo, color: 'text-emerald-600', bg: 'bg-emerald-100 dark:bg-emerald-500/20' };
        case 'productivity': return { title: 'Produktivitas', icon: BarChart3, color: 'text-amber-600', bg: 'bg-amber-100 dark:bg-amber-500/20' };
        default: return {};
    }
});
</script>
    ...
    <ManagerLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Pusat Laporan Divisi</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Export dan analisa data operasional dalam format Excel.</p>
                </div>
            </div>
        </template>
                    ...
                    <div class="max-w-2xl w-full mx-auto">
                        ...
                        <div class="flex items-center gap-5 mb-10">
                            ...
                            <div>
                                ...
                                <p class="text-slate-500 dark:text-slate-400 mt-1">Konfigurasi rentang laporan yang dingin di ekspor.</p>
                            </div>
                        </div>
